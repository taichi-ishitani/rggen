...
describe 'bit_field/type' do
        ...
        context "設定した幅以外のビットフィールドで使用した場合" do
          it "RegisterMapErrorを発生させる" do
            {1 => "[0]", 3 => "[2:0]", 32 => "[31:0]"}.each do |width, assignment|
              ...
              expect {
                @factory.create(configuration, register_map_file)
              }.to raise_register_map_error("2 bit(s) width required: #{width} bit(s)", position("block_0", 4, 4))
            end
          end
        end
      end
        ...
        context "指定された以外の幅を持つビットフィールドで使用した場合" do
          it "RGen::RegisterMapErrorを発生させる" do
            {2 => "[1:0]", 4 => "[3:0]"}.each do |width, assignment|
              ...
              expect {
                @factory.create(configuration, register_map_file)
              }.to raise_register_map_error("#{[1, 3]} bit(s) width required: #{width} bit(s)", position("block_0", 4, 4))
            end
          end
        end
      end
        ...
        context "指定された以外の幅を持つビットフィールドで使用した場合" do
          it "RGen::RegisterMapErrorを発生させる" do
            {1 => "[0]", 5 => "[4:0]"}.each do |width, assignment|
              ...
              expect {
                @factory.create(configuration, register_map_file)
              }.to raise_register_map_error("#{2..4} bit(s) width required: #{width} bit(s)", position("block_0", 4, 4))
            end
          end
        end
      end
        ...
        context "データ幅以外のビットフィールド使用した場合" do
          it "RGen::RegisterMapErrorを発生させる" do
            {1 => "[0]", 31 => "[30:0]"}.each do |width, assignment|
              ...
              expect {
                @factory.create(configuration, register_map_file)
              }.to raise_register_map_error("#{configuration.data_width} bit(s) width required: #{width} bit(s)", position("block_0", 4, 4))
            end
          end
        end
      end
        ...
        context "trueに設定されている場合" do
          ...
          context "参照信号を持たないビットフィールで使用した場合" do
            it "RGen::RegisterMapErrorを発生させる" do
              ...
              expect {
                @factory.create(configuration, register_map_file)
              }.to raise_error "reference bit field required", position("block_0", 4, 4)
            end
          end
        end
      end
    ...
    context "Rgen.enableで有効にされたタイプ以外が入力された場合" do
      it "RegisterMapErrorを発生させる" do
        ["baz", "qux"].each do |type|
          ...
          expect {
            @factory.create(configuration, register_map_file)
          }.to raise_register_map_error("unknown bit field type: #{type}", position("block_0", 4, 4))
        end
      end
    end
  end
  ...
  describe "rtl" do
    let(:register_map) do
      set_load_data([
        [nil, "register_0", "bit_field_0_0", "[31:0]", "foo", nil],
        [nil, "register_1", "bit_field_1_0", "[0]"   , "foo", nil]
      ])
      @factory.create(configuration, register_map_file)
    end
    ...
    context "アクセス属性がread-writeの場合" do
      ...
      it "value信号を持つ" do
        ...
        expect(rtl[1]).to have_logic(:value, name: 'bit_field_1_0_value', width: 1)
      end
    end
    ...
    context "アクセス属性がreservedの場合" do
      ...
      it "value信号を持つ" do
        ...
        expect(rtl[1]).not_to have_signal_declaration(name: 'bit_field_1_0_value', type: :logic, width: 16)
      end
    end
  end
end
