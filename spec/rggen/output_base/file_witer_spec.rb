  ...
  describe FileWriter do
    ...
    describe "#write_file" do
      it "コード生成ブロックをコンテキストオブジェクト上で実行した結果をファイルに書き出す" do
        (create_file_writer("test.txt") { |f| f << content }).tap do |writer|
          expect {
      ...
      it "パターンをコンテキスオブジェクトト上で実行した結果をファイル名とする" do
        (create_file_writer('<%= file_name %>.txt') { |f| f << content }).tap do |writer|
          expect {
            writer.write_file(foo_context)
        ...
        (create_file_writer('baz/<%= file_name %>.txt') { |f| f << content }).tap do |writer|
          expect {
            writer.write_file(foo_context)
      ...
      it "コンテキストオブジェクトの#create_blank_fileを呼び出して、バッファ用のソースファイルオブジェクトを生成する" do
        allow(File).to receive(:binwrite)
        expect(foo_context).to receive(:create_blank_file).and_call_original
        (create_file_writer("test.txt") { |f| f << content }).tap do |writer|
          writer.write_file(foo_context)
        end
      end
