...
module RgGen::VerilogUtility
  describe Variable do
    ...
    it "変数宣言を行うコードを生成する" do
      ...
      expect(variable(name: "foo", data_type: :logic, width: 2    , dimensions: [2   ], array_format: :vectored          )).to eq "logic [3:0] foo"
      expect(variable(name: "foo", data_type: :logic, width: 2    , dimensions: [3, 4], array_format: :vectored          )).to eq "logic [23:0] foo"
    ...
    it "ポート宣言を行うコードを生成する" do
      ...
      expect(port(name: "foo", direction: :output,                                  dimensions: [2   ], array_format: :vectored)).to eq "output [1:0] foo"
      expect(port(name: "foo", direction: :output,                                  dimensions: [3, 4], array_format: :vectored)).to eq "output [11:0] foo"
      expect(port(name: "foo", direction: :output, data_type: :logic, width: 2    , dimensions: [3, 4], array_format: :vectored)).to eq "output logic [23:0] foo"
    ...
    it "パラメータ宣言を行うコードを生成する" do
      ...
      expect(parameter(name: "FOO", parameter_type: :parameter ,                                  dimensions: [2, 1], array_format: :vectored, default: 0              )).to eq "parameter [1:0] FOO = 0"
      expect(parameter(name: "FOO", parameter_type: :parameter , data_type: :logic, width: 2    , dimensions: [2   ], array_format: :vectored, default: 0              )).to eq "parameter logic [3:0] FOO = 0"
