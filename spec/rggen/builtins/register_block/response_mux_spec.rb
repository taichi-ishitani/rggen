...
describe "register_block/response_mux" do
  ...
  context "外部レジスタを含む場合" do
    it "外部レジスタ用のセレクト信号を持つ" do
      expect(rtl[1]).to have_logic(:external_register_select, width: total_external_registers)
    end

    it "外部レジスタ用のレディ信号を持つ" do
      expect(rtl[1]).to have_logic(:external_register_ready, width: total_external_registers)
    end

    it "外部レジスタ用のステータス信号を持つ" do
      expect(rtl[1]).to have_logic(:external_register_status, width: 2, dimensions: [total_external_registers])
    end
  end
