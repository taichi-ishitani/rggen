...
describe "register_block/clock_reset" do
  ...
  it "クロック入力ポートを持つ" do
    expect(@rtl.register_blocks[0]).to have_input(:register_block, :clock, name: "clk", data_type: :logic, width:1)
  end

  it "リセット入力ポートを持つ" do
    expect(@rtl.register_blocks[0]).to have_input(:register_block, :reset, name: "rst_n", data_type: :logic, width:1)
  end
end
